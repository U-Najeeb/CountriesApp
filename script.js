...
const cardContainer = document.querySelector(".country--cards-box");
...
const dark1 = document.querySelectorAll(".dark--1");
const dark2 = document.querySelector(".dark--2");
const dark3 = document.querySelector(".dark--3");
const darkModeBtn = document.querySelector(".dark--mode-container");


// Function to render home page with country cards
const homePage = (data) => {
    cardContainer.innerHTML = " ";
    // console.log(darkModeBtn)
    const darkModeClass = darkModeBtn.classList.contains("dark--mode--btn") ? "dark--mode-1" : "";
    data.forEach(cardData => {
        const html = `
            <div class="country--card ${darkModeClass}">
                <div class="image--box">
                    <img src=${cardData.flags?.png || cardData.flags?.svg} alt="${cardData.name}" class="country--flag">
                </div>
                <div class="country--data ${darkModeClass}">
                    <h3 class="country--name">${cardData.name}</h3>
                    <div class="country--stats ${darkModeClass}">
                        <p class="population">Population: ${cardData.population}</p>
                        <p class="region">Region: ${cardData.region}</p>
                        <p class="capital">Capital: ${cardData.capital}</p>
                    </div>
                </div>
            </div>`;
...
// Function to render details page for a specific country
const detailsPage = (data) => {
    contentContainer2.innerHTML = " ";
    contentContainer2.classList.remove("hidden");
    contentContainer.classList.add("hidden");
    const darkModeClass = darkModeBtn.classList.contains("dark--mode--btn") ? "dark--mode-2" : "";
    const darkModeClass2 = darkModeBtn.classList.contains("dark--mode--btn") ? "dark--mode-1" : "";
    const bordersHtml = data.borders?.map(border => `<div class="border ${darkModeClass2}">${border}</div>`).join("");

    const html = `
        <section class="content--section-22 content--container-2 ${darkModeClass}">
            <div class="content--section--wrapper-2">
                <div class="back--btn-box">
                    <button class="back--btn ${darkModeClass2}">&larr; Back</button>
                </div>
                <div class="country--page-box">
                    <div class="country--flag--box-large">
                        <img src=${data.flags?.png || data.flags?.svg} alt="${data.name}" class="country--flag-large">
                    </div>

                    <div class="country--page--details-box">
                        <div class="country--page--heading-box">
                            <h1 class="country--page-heading">${data.name}</h1>
                        </div>
                        <div class="country--page--details">
                            <div class="country--page--details-col1">
                                <p class="native--name">Native Name: ${data.nativeName}</p>
                                <p class="population">Population: ${data.population}</p>
                                <p class="region">Region: ${data.region}</p>
                                <p class="sub--region">Sub Region: ${data.subregion}</p>
                                <p class="capital">Capital: ${data.capital}</p>
                            </div>
                            <div class="country--page--details-col2">
                                <p class="top--level-domain">Top Level Domain: ${data.topLevelDomain?.[0]}</p>
                                <p class="currency">Currency: ${data.currencies?.[0].name}</p>
                                <p class="language">Language: ${data.languages?.[0].name}</p>
                            </div>
                        </div>

                        <div class="borders--box">
                            <h2 class="borders--heading">Border Countries: </h2>
                            <div class="borders">
                                ${bordersHtml}
                            </div>
                        </div>
    ...
    contentContainer2.insertAdjacentHTML("beforeend", html);
};

// Debouncer variable for search field input
let debouncer;

// Variable to store currently selected country
let country;
...
searchFeild.addEventListener("input", (e) => {
    ...
    debouncer = setTimeout(() => {
        country = data.find((country) => (country.name).toLowerCase() === (searchFeild.value.toLowerCase()).trim());
        if (!country) {
            return alert("OOPS! Country not found");
        }
...
// Event listener for back button in details page
sectionContainer.addEventListener('click', (e) => {
    if (e.target.classList.contains('back--btn')) {
        e.preventDefault();
        contentContainer2.classList.add("hidden");
...
// Event listener for region filter button
filterBtn.addEventListener("change", (e) => {
    e.preventDefault();
    const countries = data.filter((country) => country.region === e.target.value);
    homePage(countries);
});
...
darkModeBtn.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("dark--mode--btn")) {
        dark1.forEach((ele) => {
            ele.classList.toggle("dark--mode-1");
        });

        body.classList.toggle("dark-mode")
        dark2.classList.toggle("dark--mode-2")
        homePage(data)
        
        dark3.classList.toggle("dark--mode-2")
